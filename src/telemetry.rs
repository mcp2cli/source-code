...
pub struct TelemetryEvent {
    ...
    /// What happened.
    pub event: EventKind,
    /// Optional browser-session id handed off by install.sh — only
    /// populated on the `first_run` event so we can correlate a web
    /// visit with the install that followed.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub visitor_id: Option<String>,
    /// Optional per-install UUID generated by install.sh — also
    /// only populated on `first_run`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Visitor + install IDs handed off from `install.sh` on the very
/// first run. One-shot: the installer writes the file, this recorder
/// reads it, attaches both IDs to the `first_run` event, and deletes
/// the file so subsequent runs don't keep echoing an install context
...
fn consume_install_ref(data_root: &Path) -> Option<InstallRef> {
    let path = data_root.join("install_ref");
    let content = fs::read_to_string(&path).ok()?;
    let mut visitor = None;
    let mut install = None;
    for line in content.lines() {
        if let Some(v) = line.strip_prefix("visitor_id=") {
            let trimmed = v.trim();
            if !trimmed.is_empty() {
                visitor = Some(trimmed.to_string());
            }
        } else if let Some(v) = line.strip_prefix("install_id=") {
            let trimmed = v.trim();
            if !trimmed.is_empty() {
                install = Some(trimmed.to_string());
            }
        }
    }
    // One-shot hand-off — delete regardless of what we parsed so the
...
impl TelemetryRecorder {
    ...
    pub fn new(config: &TelemetryConfig, data_root: &Path) -> Option<Self> {
        ...
        let install_ref = consume_install_ref(data_root);
        Some(Self {
            config: config.clone(),
    ...
    fn build_event(&self, event: EventKind) -> TelemetryEvent {
        // Attach the install_ref IDs to the FirstRun event only —
        // subsequent command_run events are correlated via
        // installation_id, so we don't need to keep re-emitting them.
        let (visitor_id, install_id) = match (&event, &self.install_ref) {
            (EventKind::FirstRun, Some(r)) => (r.visitor_id.clone(), r.install_id.clone()),
            _ => (None, None),
        };
        TelemetryEvent {
            schema: 1,
...
fn event_to_span(event: &TelemetryEvent) -> serde_json::Value {
    ...
    if let Some(v) = &event.visitor_id {
        attributes.push(str_attr("mcp2cli.visitor_id", v));
    }
    if let Some(i) = &event.install_id {
        attributes.push(str_attr("mcp2cli.install_id", i));
    }
